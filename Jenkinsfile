pipeline {
    ...
    environment {
        DOCKER_IMAGE_NAME = 'atefbh/foyerapp'
    ...
    stages {
        stage('Git') {
            steps {
                ...
                git branch: 'moduleBloc-atefbenharb', url: 'https://github.com/houssem112c/devops-.git'
            }
        }
    ...
    steps {
        ...
        stage('Build & Push Docker Image') {
            steps {
                script {
                    dir("${WORKSPACE}") {
                        sh "docker build -t atefbh/foyerapp:latest ."
                    sh "docker push atefbh/foyerapp:latest"
                    }
                }
            }
        }
